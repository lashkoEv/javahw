...
public class AcademyService {
    ...
    public List<StudentDto> findStudentsDto(Integer size) {
        return studentRepository.findAll().stream()
                                .filter(student -> student.getId() <= size && student.getId() >= size - 4)
                                .map(s -> new StudentDto(s.getId(), s.getFirstName(), s.getLastName(), s.getPhoto().substring(s.getPhoto().lastIndexOf("/") + 1), s.getBirthDate(), s.getGroup().getId(), s.getGroup().getName()))
                                .collect(Collectors.toList());
    }

    public List<StudentDto> findStudentsByFirstOrLastName(String firstName, String lastName) {
        return studentRepository.findAll().stream()
    ...
    public List<StudentDto> findStudentsByFirstAndLastName(String firstName, String lastName) {
        return studentRepository.findAll().stream()
                                          .filter(student -> student.getFirstName().equals(firstName) && student.getLastName().equals(lastName))
                                          .map(s -> new StudentDto(s.getId(), s.getFirstName(), s.getLastName(), s.getPhoto().substring(s.getPhoto().lastIndexOf("/") + 1), s.getBirthDate(), s.getGroup().getId(), s.getGroup().getName()))
                                          .collect(Collectors.toList());
    ...
    public List<GroupDto> findGroupsDto(Integer size) {
        return groupRepository.findAll().stream()
                                        .filter(group -> group.getId() <= size && group.getId() >= size - 4).map(g -> new GroupDto(g.getId(), g.getName()))
                                        .collect(Collectors.toList());
    }

    public List<GroupDto> findGroupsByName(String name) {
        return groupRepository.findAll().stream()
                                        .filter(group -> group.getName().equals(name)).map(g -> new GroupDto(g.getId(), g.getName()))
                                        .collect(Collectors.toList());
    }

    public List<TeacherDto> findAllTeachers(Integer size) {
        return teacherRepository.findAll().stream()
                                          .filter(teacher -> teacher.getId() <= size && teacher.getId() >= size - 4)
                                          .map(t -> new TeacherDto(t.getId(), t.getFirstName(), t.getLastName(), t.getGroups().stream().map(Group::getId).collect(Collectors.toList())))
                                          .collect(Collectors.toList());
    }

    public List<TeacherDto> findTeachersByFirstOrLastName(String firstName, String lastName) {
        return teacherRepository.findAll().stream()
                                          .filter(teacher -> teacher.getFirstName().equals(firstName) || teacher.getLastName().equals(lastName))
                                          .map(t -> new TeacherDto(t.getId(), t.getFirstName(), t.getLastName(), t.getGroups().stream().map(Group::getId).collect(Collectors.toList())))
                                          .collect(Collectors.toList());
    }

    public List<TeacherDto> findTeachersByFirstAndLastName(String firstName, String lastName) {
        return teacherRepository.findAll().stream()
